...
export function handleRecordMatch(event: SubmitEvent, context: MatchContext) {
    ...
    try {
        ...
        handleError(error, {
            context: 'RecordMatch',
            userMessage: 'Failed to record match'
        });
    }
}

export function handleClearMatchHistory(context: MatchContext) {
    if (confirm('Are you sure you want to clear all match history? This action cannot be undone.')) {
        try {
            ...
            context.render();
            showNotification('Match history cleared and player stats reset.', 'success');
        } catch (error) {
            handleError(error, {
                context: 'ClearMatchHistory',
                userMessage: 'Failed to clear match history'
            });
        }
    }
}
